...
try:
    from ....pospyt.pospyt.pospyt import PosWebServiceError
except ImportError:
...
class SaleImportRule(Component):
    ...
    def _get_paid_amount(self, record):
        payment_adapter = self.component(
            usage="backend.adapter", model_name="__not_exist_pos.payment"
        )
...
class SaleOrderImporter(Component):
    ...
    def _import_dependencies(self):
        ...
        if record["id_carrier"] != "0":
            self._import_dependency(record["id_carrier"], "pos.delivery.carrier")
        ...
        for row in rows:
            try:
                self._import_dependency(
                    row["product_id"], "pos.product.template"
                )
            ...
            if row.get("product_attribute_id", "0") != "0":
                try:
                    self._import_dependency(
                        row["product_attribute_id"], "pos.product.combination"
                    )
                except PosWebServiceError as err:
                    # we ignore it, the order line will be imported without
                    # product
                    _logger.error(
                        "Pos combination %s could not be imported, error: %s",
...
class SaleOrderLineMapper(Component):
    ...
    def product_id(self, record):
        if int(record.get("product_attribute_id", 0)):
            combination_binder = self.binder_for("pos.product.combination")
